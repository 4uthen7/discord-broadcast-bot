...
Bot に「メッセージを送る」権限がない状態です。次の2つを確認してください。

- サーバー全体の権限: サーバー設定 > ロール で、Bot のロール (Bot と同じ名前のロール) に「メッセージを送る」と「チャンネルを表示」を許可します。招待 URL をもう一度開いて認可し直すと、Bot のロール権限が更新されます。
- チャンネル個別の権限: 読み取り専用のチャンネルでは @everyone に「メッセージを送る」が拒否されていることがあります。チャンネル設定 > 権限 で Bot を追加し、「メッセージを送る」を許可してください。

確認画面と実行結果には、Bot の権限の内訳 (チャンネルを表示 / メッセージを送る / 全員宛にメンション) と不足している権限が表示されます。
