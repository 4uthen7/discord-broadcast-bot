...
- Bot は人間のように招待リンクからサーバーへ参加することはできません。OAuth2 の認可 (追加操作) が必要で、これには追加する人に「サーバー管理」権限が要ります。参加が自由な公開サーバーでも同じです。
- 自分で作ったサーバーならオーナーなので自由に追加できます。他人のサーバーで使いたい場合は、そのサーバーの「サーバー管理」を持つ人に上の URL を渡して追加してもらってください。
- ポータルの インストール タブ > ギルドのインストール > スコープ に bot を追加し、権限に「チャンネルを表示」「メッセージを送る」(全員宛メンションを使うなら「全員宛にメンション」) を設定しておくと、Discord 提供のインストールリンクでも Bot 本体が追加されるようになります。既定では applications.commands だけなので Bot 本体が追加されません。
...
## ユーザーインストール (Bot をサーバーに入れずに使う)

サーバー管理権限を持っていないと、Bot をそのサーバーへ追加することはできません (Discord の仕様)。その代わりに、アプリを **あなたのアカウントに追加** して、参加しているサーバーならどこでも使えるようにする「ユーザーインストール」に対応しています。

- 追加リンク: https://discord.com/oauth2/authorize?client_id=<CLIENT_ID>&integration_type=1&scope=applications.commands
- 追加後は、あなたが参加しているサーバーで /broadcast を実行できます (Bot をサーバーに入れる必要はありません)
- 使えるもの: target=everyone / here / none での送信
- 使えないもの: メンバー一覧を取得できないため target=members / send_style=per_member は使えません。また、応答トークンの都合で長い interval の連続送信は途中で切れます
- @everyone メンションが通るかは、あなたの権限とチャンネル設定によります

## 実行できる人を許可する (任意)

既定では「サーバー管理 / 管理者 / メンション @everyone」のいずれかを持つ人だけが /broadcast を実行できます。サーバーの管理者ではない人にも使わせたい場合は、環境変数で許可リストを追加します。

- EXECUTOR_USER_IDS: 実行を許可するユーザー ID (カンマ区切り)
- EXECUTOR_ROLE_IDS: 実行を許可するロール ID (カンマ区切り)

許可リストを設定すると、コマンドが全員に表示されるようになり、実行の可否をコード側で判定します。Bot が参加しているサーバーであれば、サーバー管理権限を持っていない人でも実行できます。
